...
import { DIG_LEVELS, LEVELS, STARTER } from './levels';
...
function drawn(lvl: LevelDef): string[] {
  ...
  // Pipes, bombs and gate taps count as marks too.
  for (const a of lvl.solution) rows[a.at.y][a.at.x] = a.type === 'dig' ? '*' : a.type === 'fill' ? 'x' : a.type[0];
...
describe('level registry', () => {
  it('has the 6 lessons, Dry Maze, 100 more and the tool levels, with unique ids and names', () => {
    expect(DIG_LEVELS).toHaveLength(107);
    expect(LEVELS).toHaveLength(107 + TOOL_LEVELS.length);
